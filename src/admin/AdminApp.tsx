...
function Support() {
  const [rows, setRows] = useState<Row[]>([]); const [state, setState] = useState<LoadState>('loading'); const [error, setError] = useState(''); const [selected, setSelected] = useState<Row | null>(null);
  const reload = useCallback(async () => {
    if (!supabase) return; setState('loading'); setError('');
    const { data, error: queryError } = await supabase.rpc('admin_support_inbox');
    if (queryError) { setError('تعذر تحميل صندوق الدعم. تأكد من تطبيق ترقية هوية المرسل الآمنة.'); setState('error'); return; }
    const nextRows = (data ?? []) as Row[]; setRows(nextRows); setSelected(current => current ? nextRows.find(row => row.id === current.id) ?? null : current); setState('ready');
  }, []);
  useEffect(() => { reload(); const client = supabase; if (!client) return; const channel = client.channel('admin-support-inbox').on('postgres_changes', { event: '*', schema: 'public', table: 'support_conversations' }, reload).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'support_messages' }, reload).subscribe(); return () => { client.removeChannel(channel); }; }, [reload]);
  const subjectLabel = (value: unknown) => value === 'general' ? 'محادثة عامة' : typeof value === 'string' && value.trim() ? value : 'محادثة دعم';
  return <Page title="الدعم" subtitle="رسائل العملاء وردود فريق مر، بهوية مرسل واضحة."><div className="support-layout"><section className="support-list"><div className="support-list-head"><span><Inbox/><b>صندوق الوارد</b></span><button className="icon-button" onClick={reload} aria-label="تحديث المحادثات"><RefreshCw/></button></div>{state === 'error' ? <DataError message={error}/> : state === 'loading' ? <LoadingRows/> : rows.map(row => { const unread = Number(row.unread_customer_count ?? 0); return <button key={String(row.id)} className={selected?.id === row.id ? 'active' : ''} onClick={() => setSelected(row)}><div className="support-avatar"><UserRound/></div><span><b>{text(row,'customer_name','masked_phone').replace('—','عميل مر')}</b><small>{subjectLabel(row.subject)} · {date(row.updated_at)}</small><em>{text(row,'last_message')}</em></span><div className="support-list-meta"><Status value={String(row.status)}/>{unread > 0 && <strong>{unread}</strong>}</div></button>; })}{state === 'ready' && !rows.length && <div className="empty-state"><MessageCircle/><b>لا توجد محادثات دعم</b></div>}</section><section className="conversation-panel">{selected ? <Conversation conversation={selected} onChanged={reload}/> : <div className="empty-state"><Headphones/><b>اختر محادثة</b><p>ستظهر الرسائل والردود هنا بوضوح.</p></div>}</section></div></Page>;
}

function Conversation({ conversation, onChanged }: { conversation: Row; onChanged: () => void }) {
  const [messages, setMessages] = useState<Row[]>([]); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [sendError, setSendError] = useState(''); const [status, setStatus] = useState(String(conversation.status));
  useEffect(() => setStatus(String(conversation.status)), [conversation.status]);
  const load = useCallback(async () => { if (!supabase) return; const { data } = await supabase.from('support_messages').select('*').eq('conversation_id', conversation.id).order('created_at'); setMessages((data ?? []) as Row[]); await supabase.rpc('mark_support_read', { p_conversation_id: String(conversation.id) }); }, [conversation.id]);
  ...
  const send = async (event: FormEvent) => { event.preventDefault(); if (!supabase || !message.trim()) return; setBusy(true); setSendError(''); const { error } = await supabase.rpc('staff_send_support_message', { p_conversation_id: String(conversation.id), p_message: message.trim() }); if (!error) { setMessage(''); await load(); onChanged(); } else setSendError('تعذر إرسال الرد. حاول مرة أخرى.'); setBusy(false); };
  const toggle = async () => { if (!supabase) return; const next = status === 'open' ? 'closed' : 'open'; const { error } = await supabase.rpc('set_support_conversation_status', { p_conversation_id: String(conversation.id), p_status: next }); if (!error) { setStatus(next); onChanged(); } };
  const customerName = text(conversation,'customer_name','masked_phone').replace('—','عميل مر');
  return <div className="conversation"><header><div className="conversation-customer"><span className="support-avatar"><UserRound/></span><span><b>{customerName}</b><small><Clock3/> بدأت {date(conversation.created_at)}</small></span></div><div className="conversation-actions"><Status value={status}/><button onClick={toggle}>{status === 'open' ? 'إغلاق المحادثة' : 'إعادة فتح المحادثة'}</button></div></header><div className="messages">{messages.map(row => { const kind = String(row.sender_type); const label = kind === 'customer' ? customerName : kind === 'system' ? 'مر' : 'فريق دعم مر'; return <article key={String(row.id)} className={`message message-${kind}`}><b>{label}</b><p dir="auto">{text(row,'message')}</p><small>{date(row.created_at)}{row.read_at && kind !== 'customer' ? ' · مقروءة' : ''}</small></article>; })}{!messages.length && <div className="empty-state"><MessageCircle/><b>لا توجد رسائل</b></div>}</div><form onSubmit={send}><div><input value={message} onChange={event => setMessage(event.target.value)} placeholder="اكتب رد فريق دعم مر…" dir="auto"/><small>سيظهر الرد للعميل باسم «دعم مر».</small>{sendError && <span className="admin-error">{sendError}</span>}</div><button className="admin-primary" disabled={busy || !message.trim()}><Send/>{busy ? 'جارٍ الإرسال…' : 'إرسال'}</button></form></div>;
}
